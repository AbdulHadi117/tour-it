  ...
  return (
    <section className="rounded-[28px] border border-[rgba(18,35,58,0.08)] bg-white p-6 shadow-[0_16px_40px_rgba(18,35,58,0.05)]">
      <div className="mb-5 flex items-center justify-between gap-3">
        <h3 className="text-lg font-bold text-[#12233A]" style={{ fontFamily: "'Playfair Display', serif" }}>
          {title}
        </h3>
      </div>
      {children}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1.1fr_0.9fr]">
            <div className="p-8 lg:p-10 relative overflow-hidden">
              ...
              <div className="relative space-y-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-[11px] font-bold uppercase tracking-[0.3em] text-white/55">Account profile</p>
                    <h1 className="mt-2 text-3xl lg:text-4xl font-black" style={{ fontFamily: "'Playfair Display', serif" }}>
                      {draft.fullName}
                    </h1>
                ...
                <p className="max-w-2xl text-sm lg:text-base leading-7 text-white/75">
                  Keep your travel identity, account details, and notification preferences in one place. Changes here sync with your saved profile on this device.
                </p>
            ...
            <div className="bg-white/5 p-8 lg:p-10 border-t lg:border-t-0 lg:border-l border-white/10 flex flex-col gap-4">
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2">
                {[
                  { icon: <ShieldCheck size={16} />, label: "Profile synced", value: "On" },
                  { icon: <Bell size={16} />, label: "Safety alerts", value: draft.safetyAlerts ? "Enabled" : "Off" },
                  { icon: <Bookmark size={16} />, label: "Newsletter", value: draft.newsletter ? "Subscribed" : "Off" },
                  { icon: <Star size={16} />, label: "Travel style", value: draft.travelStyle },
                ].map((item) => (
                  <div key={item.label} className="rounded-2xl border border-white/10 bg-white/8 px-4 py-4">
                    <div className="flex items-center gap-2 text-white/70">{item.icon}<span className="text-xs uppercase tracking-widest">{item.label}</span></div>
                    <p className="mt-2 text-sm font-bold text-white">{item.value}</p>
                  </div>
                ))}
              </div>
          ...
          <SectionCard title="Edit profile">
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="space-y-2 sm:col-span-2">
                <span className="text-xs font-bold uppercase tracking-widest text-[#12233A]">Full name</span>
                <input value={draft.fullName} onChange={(event) => setDraft({ ...draft, fullName: event.target.value, avatarSeed: deriveAvatarSeed(event.target.value) })} className="w-full rounded-2xl border border-[rgba(18,35,58,0.12)] bg-[#FAF8F3] px-4 py-3 text-sm outline-none focus:border-[#0E8C88]" />
              </label>
              <label className="space-y-2">
                <span className="text-xs font-bold uppercase tracking-widest text-[#12233A]">Email</span>
                <input value={draft.email} onChange={(event) => setDraft({ ...draft, email: event.target.value })} className="w-full rounded-2xl border border-[rgba(18,35,58,0.12)] bg-[#FAF8F3] px-4 py-3 text-sm outline-none focus:border-[#0E8C88]" />
              </label>
              <label className="space-y-2">
                <span className="text-xs font-bold uppercase tracking-widest text-[#12233A]">Phone</span>
                <input value={draft.phone} onChange={(event) => setDraft({ ...draft, phone: event.target.value })} className="w-full rounded-2xl border border-[rgba(18,35,58,0.12)] bg-[#FAF8F3] px-4 py-3 text-sm outline-none focus:border-[#0E8C88]" />
              </label>
              <label className="space-y-2 sm:col-span-2">
                <span className="text-xs font-bold uppercase tracking-widest text-[#12233A]">Home base</span>
                <input value={draft.location} onChange={(event) => setDraft({ ...draft, location: event.target.value })} className="w-full rounded-2xl border border-[rgba(18,35,58,0.12)] bg-[#FAF8F3] px-4 py-3 text-sm outline-none focus:border-[#0E8C88]" />
              </label>
              <label className="space-y-2 sm:col-span-2">
                <span className="text-xs font-bold uppercase tracking-widest text-[#12233A]">Bio</span>
                <textarea value={draft.bio} onChange={(event) => setDraft({ ...draft, bio: event.target.value })} rows={4} className="w-full rounded-2xl border border-[rgba(18,35,58,0.12)] bg-[#FAF8F3] px-4 py-3 text-sm outline-none focus:border-[#0E8C88]" />
              </label>
            </div>
          </SectionCard>
            ...
            <SectionCard title="Preferences">
              <div className="space-y-4">
                <label className="block space-y-2">
                  <span className="text-xs font-bold uppercase tracking-widest text-[#12233A]">Travel style</span>
                  <select value={draft.travelStyle} onChange={(event) => setDraft({ ...draft, travelStyle: event.target.value })} className="w-full rounded-2xl border border-[rgba(18,35,58,0.12)] bg-[#FAF8F3] px-4 py-3 text-sm outline-none focus:border-[#0E8C88]">
                ...
                <label className="block space-y-2">
                  <span className="text-xs font-bold uppercase tracking-widest text-[#12233A]">Languages</span>
                  <input value={draft.languages.join(", ")} onChange={(event) => setDraft({ ...draft, languages: event.target.value.split(",").map((item) => item.trim()).filter(Boolean) })} className="w-full rounded-2xl border border-[rgba(18,35,58,0.12)] bg-[#FAF8F3] px-4 py-3 text-sm outline-none focus:border-[#0E8C88]" />
                </label>
                <label className="flex items-center justify-between gap-3 rounded-2xl border border-[rgba(18,35,58,0.12)] px-4 py-3">
                  <span>
                    <span className="block text-sm font-semibold text-[#12233A]">Newsletter</span>
                    <span className="block text-xs text-[#6B7280]">Product updates and destination drops.</span>
                  </span>
                  <input checked={draft.newsletter} onChange={(event) => setDraft({ ...draft, newsletter: event.target.checked })} type="checkbox" className="h-4 w-4 rounded border-[rgba(18,35,58,0.2)] text-[#0E8C88] focus:ring-[#0E8C88]" />
                </label>
                <label className="flex items-center justify-between gap-3 rounded-2xl border border-[rgba(18,35,58,0.12)] px-4 py-3">
                  <span>
                    <span className="block text-sm font-semibold text-[#12233A]">Safety alerts</span>
                    <span className="block text-xs text-[#6B7280]">Road conditions and trip interruption notices.</span>
                  </span>
                  <input checked={draft.safetyAlerts} onChange={(event) => setDraft({ ...draft, safetyAlerts: event.target.checked })} type="checkbox" className="h-4 w-4 rounded border-[rgba(18,35,58,0.2)] text-[#0E8C88] focus:ring-[#0E8C88]" />
                </label>
              </div>
            </SectionCard>
            ...
            <SectionCard title="Account security">
              <div className="space-y-4">
                <div className="rounded-2xl bg-[#FAF8F3] p-4">
                  <div className="flex items-center gap-2 text-sm font-semibold text-[#12233A]"><UserRound size={15} className="text-[#0E8C88]" /> {draft.email}</div>
                  <p className="mt-1 text-xs text-[#6B7280]">Primary login email</p>
                </div>
                <div className="rounded-2xl bg-[#FAF8F3] p-4">
